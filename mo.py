# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 22:34:38 2018
...
def calc_density(Position, ncells, L):
    ...
    Position        = Position/dx           # Positioning 
    LowerPos        = np.int_(Position)     #Lower Positioning 
    Offset          = Position - LowerPos   # Offset from the left    
    Offset_sh       = 1 - Offset            #The offset shift 
    
    Density         = zeros((ncells))
    
    for i in range(0, ncells):                      # Loop over all cell numbers
        Sid         = np.where((LowerPos == i))     # Returns values which are equal to i for LowerPos
        
        Density[(i+1)%ncells] += sum(Offset[Sid])
        Density[i] += sum(Offset_sh[Sid])
                                                    # nparticles now distributed amongst ncells
    
    Density *= float(ncells) / float(len(Position)) # Make average density equal to 1
    return Density
