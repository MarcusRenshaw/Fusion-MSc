...
def calc_density(position, ncells, L):
    ...
    cell_numbers = position / dx
    
    nearest_left_cell = [int(p) for p in cell_numbers]
    offset = map(operator.sub, cell_numbers, nearest_left_cell)
    ...
    def get_left_index(x):
        left_of_index = int(x)
        return left_of_index

    def get_offset(x,left_of_x):
        offset = x - left_of_x
        return offset
